...
`RunPodClient.provision_pod` now escapes env values before they go into the pod
GraphQL mutation. The SDK's mutation generator interpolates `value: "{v}"`
without escaping, so an env value containing a double quote — e.g. the
`HYPERPARAMS_JSON` blob, which is JSON — broke the query (`Syntax Error: Expected
":"`). Backslash, quote and newline are now escaped, so any env value survives.
